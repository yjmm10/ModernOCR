...
function(extract_file filename extract_dir)
  ...
  # cmake -E tar -xf filename.tgz   xzf:.zip
...
function(get_file url filename extract_dir)
...
function(download_and_extract)
  ...
  #   message(${_EXPECT_HASH} ${_ACTUAL_CHKSUM})
...
macro(FIND_INCLUDE_DIR result curdir)  #定义函数,2个参数:存放结果result；指定路径curdir；
    file(GLOB_RECURSE children "${curdir}/*.lib" "${curdir}/*.h" )	#遍历获取{curdir}中*.hpp和*.h文件列表
    ...
    endforeach()													#结束for循环
    set(${result} ${dirlist} PARENT_SCOPE)											#dirlist结果放入result变量中
    MESSAGE("ddxscgdadsgdsaaaaaaa ${result}")
endmacro()			


function(get_file_name curdir format file_list)#函数结束
    file(GLOB LIB_NAME ${curdir}/*.${format})
    # set(files "")
    FOREACH(FILE_PATH ${LIB_NAME})
        STRING(REGEX REPLACE ".+/(.+\\..*)" "\\1" FILE_NAME ${FILE_PATH})
        LIST(APPEND files ${FILE_NAME})
        MESSAGE("后面${FILE_NAME}")
    ENDFOREACH(FILE_PATH)

    set(${file_list} ${files} PARENT_SCOPE)
endfunction()